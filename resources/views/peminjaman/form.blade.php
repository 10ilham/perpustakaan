                            ...
                            <form action="{{ route('peminjaman.pinjam') }}" method="POST">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <small class="form-text" style="color: #6c757d; opacity: 0.7;">
                                        *Nama peminjam otomatis terisi sesuai akun Anda, jangan diubah.
                                    </small>
                                    @error('nama')
...
@section('scripts')
    <script>
        ...
        document.getElementById('tanggal_pinjam').addEventListener('change', function() {
            ...
            tanggalKembali.min = tanggalMin.toISOString().split('T')[0];
            tanggalKembali.max = tanggalMax.toISOString().split('T')[0];

            // Reset tanggal kembali jika tanggal yang dipilih tidak valid
            const currentKembali = new Date(tanggalKembali.value);
            if (currentKembali < tanggalMin || currentKembali > tanggalMax) {
                tanggalKembali.value = tanggalMin.toISOString().split('T')[0];
            }
        });

        // Trigger event pada saat halaman dimuat
        document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('tanggal_pinjam').dispatchEvent(new Event('change'));
        });
    </script>
@endsection
